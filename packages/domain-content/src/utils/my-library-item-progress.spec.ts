...
import { getMyLibraryItemPercent } from "./my-library-item-progress";
...
describe("getMyLibraryItemPercent", () => {
  it("uses the lesson-count rollup when present", () => {
    const percent = getMyLibraryItemPercent({
      ...baseItem,
      totalLeafCount: 5,
  ...
  it("treats a missing completedLeafCount as zero", () => {
    const percent = getMyLibraryItemPercent({ ...baseItem, totalLeafCount: 4 });
  ...
  it("returns null when neither rollup nor duration/position is available", () => {
    expect(getMyLibraryItemPercent({ ...baseItem })).toBeNull();
  });
});
