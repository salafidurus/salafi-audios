import { buildSectionTabPath } from "./get-current-section.web";
...
describe("buildSectionTabPath", () => {
  ...
  it("appends non-default feed tab", () => { expect(buildSectionTabPath("feed", "recent")).toBe("/feed/recent"); });
  ...
  it("appends non-default library tab", () => { expect(buildSectionTabPath("library", "saved")).toBe("/library/saved"); });
  ...
  it("appends non-default account tab", () => { expect(buildSectionTabPath("account", "profile")).toBe("/account/profile"); });
  it("collapses default live tab to /live", () => { expect(buildSectionTabPath("live")).toBe("/live"); });
});