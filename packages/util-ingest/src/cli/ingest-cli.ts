...
const DEFAULT_AUDIO_DIR = "packages/util-ingest/content/audio";
...
export function parseArgs(argv: string[]): IngestCliArgs {
  ...
  if (!result.tag.trim()) {
    throw new Error(
      "Missing required --tag. Example: bun run ingest:content -- --tag phase-02-seed",
    );
  }
...
function resolveCandidateInputPaths(filePath?: string): string[] {
  ...
  return [
    ...
    path.resolve(process.cwd(), "packages/util-ingest/content/phase-02-sample.json"),
  ];
}
