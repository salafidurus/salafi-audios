...
import { LocaleSchema, UpdateTopicTranslationDtoSchema } from '@sd/core-contracts';
...
} from './dto/save-topic-translation.dto';
...
export class TopicsTranslationsController {
  ...
  upsertTranslation(
    @Param('id') id: string,
    @Body({ schema: SaveTopicTranslationDtoSchema }) dto: SaveTopicTranslationDto,
  ) {
  ...
  updateTranslation(
    ...
    @Body({ schema: UpdateTopicTranslationDtoSchema }) body: Partial<{ name: string }>,
  ) {
