import type { AppThemeWeb } from "./index";

/**
 * Pure CSS-variable string builders. The web app consumes design tokens via CSS
 * custom properties (see `src/app/theme-css.ts`). Accent themes only re-declare
 * color-bearing properties, so typography/spacing/radius/shadow values stay shared
...
export const getColorThemeProperties = (theme: AppThemeWeb): string => `
  ...
  --input-surface-rest: ${theme.colors.surface.subtle};
  --input-surface-focus: ${theme.colors.surface.default};
  --input-border-rest: ${theme.recipes.chrome.inputBorderRest};
  --input-border-focus: ${theme.colors.border.primary};
