...
export function TopAuthStrip() {
  ...
  return (
    <div className={styles.strip} aria-label={t("authStrip.topActions", "Top actions")}>
          ...
          {isAuthenticated && !isLoading && user ? (
            <>
              <span style={{ fontSize: "0.875rem", color: "#6b7280" }}>
                {user.name ?? user.email ?? t("account.title", "Account")}
              </span>
              <Button
                variant="ghost"
                size="sm"
            ...
            <>
              ...
              <Button variant="primary" size="sm" onClick={() => push(routes.signUp)}>
                {t("authStrip.createAccount", "Create Free Account")}
              </Button>
            </>
          ) : null}
        </div>
