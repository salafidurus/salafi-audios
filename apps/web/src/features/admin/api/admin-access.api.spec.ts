...
vi.mock("@sd/core-contracts", () => ({
  ...
  endpoints: { admin: { users: { access: (id: string) => `/admin/users/${id}/access` } } },
}));
...
describe("aggregate access API", () => {
  ...
  it("fetches the user access snapshot", async () => {
    ...
    expect(mockHttpClient).toHaveBeenCalledWith({ url: "/admin/users/u1/access", method: "GET" });
  });
