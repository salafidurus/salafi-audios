...
export function NowPlayingBar({ title, scholar, progressPercent = 34 }: NowPlayingBarProps) {
  ...
  useLayoutEffect(() => {
    ...
    const varName = "--floating-player-height";

    const prevBodyPaddingBottom = document.body.style.paddingBottom;
    // Keep page content (incl. footer) above the fixed player.
    // The player height includes safe-area padding, so we only need the measured height.
    // Pull content down by the measured height, but shave a couple pixels off to
    // avoid a visible 1px seam from rounding/sub-pixel differences across browsers.
    document.body.style.paddingBottom = `max(0px, calc(var(${varName}, 0px) - 2px))`;

    const update = () => {
      const raw = Math.max(0, el.getBoundingClientRect().height);
      // Avoid 1px gaps from rounding across browsers/DPIs.
  ...
  return (
    <div ref={wrapRef} className={styles.floatingWrap} data-floating-player="true">
