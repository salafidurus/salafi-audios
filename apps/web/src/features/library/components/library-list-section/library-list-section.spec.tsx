...
vi.mock("@/shared/components/InfiniteScrollList", () => ({
  InfiniteScrollList: ({
    ...
    isError: boolean;
    emptyMessage: string;
    renderItem: (item: LibraryItemDto) => React.ReactNode;
  }) => (
...
const defaultProps = {
  ...
  authCopy: {
    ...
    loadingMessage: "Checking your library…",
  },
};

describe("LibraryListSection", () => {
  it("keeps the sign-in state hidden while authentication resolves", () => {
    render(<LibraryListSection {...defaultProps} authState="loading" />);
  ...
  it("renders the auth-required state after an unauthenticated result", () => {
    render(<LibraryListSection {...defaultProps} authState="unauthenticated" />);
  ...
  it("delegates authenticated loading, error, empty, and item states", () => {
    const { rerender } = render(
      <LibraryListSection {...defaultProps} query={{ ...defaultProps.query, isLoading: true }} />,
    );
    expect(screen.getByText("Loading lessons")).toBeInTheDocument();

    rerender(
      <LibraryListSection {...defaultProps} query={{ ...defaultProps.query, isError: true }} />,
    );
    expect(screen.getByText("Could not load lessons")).toBeInTheDocument();

    rerender(
      <LibraryListSection {...defaultProps} query={{ ...defaultProps.query, items: [] }} />,
    );
    expect(screen.getByText("Nothing here yet.")).toBeInTheDocument();

    rerender(<LibraryListSection {...defaultProps} />);
