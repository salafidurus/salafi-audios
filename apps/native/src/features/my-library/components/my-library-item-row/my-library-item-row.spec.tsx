...
describe("MyLibraryItemRow", () => {
  ...
  it("shows progress bar for progress variant", async () => {
    await render(<MyLibraryItemRow item={baseItem} variant="progress" />);
    expect(screen.getByTestId("my-library-progress-bar")).toBeTruthy();
  });
  ...
  it("calls onPress when pressed", async () => {
    ...
    await render(<MyLibraryItemRow item={baseItem} variant="saved" onPress={onPress} />);
  ...
  it("opens a long-press menu and reports the pressed action when actions are provided", async () => {
    ...
    await render(
      <MyLibraryItemRow
        item={baseItem}
        variant="saved"
        testID="my-library-item-row-item-1"
        actions={[{ id: "remove", title: "Remove from Saved" }]}
        onAction={onAction}
      />,
    );

    await fireEvent.press(screen.getByTestId("my-library-item-row-item-1-action-remove"));
