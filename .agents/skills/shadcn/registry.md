...
- **Built registry**: generated JSON files served to CLI consumers, usually
  from `public/r`. Use `bunx --bun shadcn@latest build` to create this form.
...
bunx --bun shadcn@latest build registry.json --output public/r
```

Use CLI commands to inspect the result:

```bash
bunx --bun shadcn@latest list @acme
bunx --bun shadcn@latest search @acme -q "login"
bunx --bun shadcn@latest view @acme/login-form
bunx --bun shadcn@latest add @acme/login-form --dry-run
...
```bash
bunx --bun shadcn@latest list owner/repo
...
bunx --bun shadcn@latest registry validate owner/repo
```
