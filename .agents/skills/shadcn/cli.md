...
> **IMPORTANT:** Always run commands using the project's package runner: `bunx --bun shadcn@latest`, `bunx --bun shadcn@latest`, or `bunx --bun shadcn@latest`. Check `packageManager` from project context to choose the right one. Examples below use `bunx --bun shadcn@latest` but substitute the correct runner for the project.
...
```bash
bunx --bun shadcn@latest apply [preset] [options]
```
...
Fuzzy search across registries. Also aliased as `bunx --bun shadcn@latest list`.
...
> Use `bunx --bun shadcn@latest apply --preset <code>` when overwriting an existing project's preset.

## Switching Presets

Ask the user first: **overwrite**, **merge**, or **skip** existing components?

- **Overwrite / Re-install** → `bunx --bun shadcn@latest apply --preset <code>`. Overwrites all detected component files with the new preset styles. Use when the user hasn't customized components.
