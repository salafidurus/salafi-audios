...
Every comment or issue posted to the issue tracker during triage **must** start with this disclaimer. Every state-changing or reconciliation action must append a comment; read-only inspection may remain comment-free.
...
When triage changes labels or state, closes or reopens an issue, records
completion or abandonment, or reconciles a merged ticket or specification, it
must append a concise comment describing the action, evidence, resulting state,
and any uncertainty. The comment must begin with the required AI-triage
disclaimer. Do not add comments for purely read-only checks unless a problem
requires recording.

Use this shape for a state-changing or reconciliation comment:
...
**Action:** Moved from `ready-for-agent` to `in-progress`
**Evidence:** Approved implementation plan; issue has one category and one state role
**Result:** State is now `in-progress`; existing artifact labels preserved
**Uncertainty:** None
```

For completion, replace the action and evidence with the merged PR number and
final commit, and state that the issue is closed with active triage labels
removed. For abandonment, record the decision and reason while preserving the
distinction from successful completion.
