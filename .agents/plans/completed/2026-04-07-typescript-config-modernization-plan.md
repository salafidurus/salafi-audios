...
- **Summary:** All stages complete. TypeScript version consolidated to `^5.9.0`, base configs
  normalized, baseUrl removed, editor/build config separated, module resolution standardized
  by workspace family.
...
- Stage 2: Canonical base configs normalized in `packages/util-config/tsconfig/`.
- Stage 3: No baseUrl entries existed; orphaned paths map removed from api tsconfig.build.json.
- Stage 4: Editor/build config separation for `apps/api` and `packages/core-contracts`.
- Stage 5: Module resolution standardized by workspace family (Bundler for frontend/packages,
  NodeNext for util-ingest, CommonJS for api).
- Stage 6: Final verification — all api typechecks, builds, and tests pass.
...
1. Stage 1: Inventory and policy definition — Done ✅
2. Stage 2: Establish canonical base configs in util-config/tsconfig/ — Done ✅
3. Stage 3: Remove baseUrl deprecation from workspace tsconfigs — Done ✅
4. Stage 4: Separate editor config from build config — Done ✅
5. Stage 5: Standardize module resolution by workspace family — Done ✅
6. Stage 6: Final verification matrix — Done ✅
