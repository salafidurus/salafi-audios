...
The active maximum is **7**. The budget may only decrease after a complete scan
reports zero violations at the current maximum. The budget was lowered through
staged, behavior-preserving refactors from an initial maximum of 100.
...
functions before and after the change. A lower threshold may be retained when
reaching it consistently produces less clear code.
